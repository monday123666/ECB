...
"""
Direct Prompts → ComfyUI REST (FLUX template-preserving)
- Use the FLUX workflow (JSON) exported from the GUI as-is
- Do NOT modify sampler, steps, CFG, negative prompt, or resolution
- Only replace the positive prompt text and SaveImage filename_prefix
- Filename pattern: {model_name}_{country}_{category}_{subcategory}_{variant}.png
"""
...
TEMPLATE_PATH = "workflow/flux_schnell_workflow.json" # FLUX template JSON exported from the GUI
OUT_DIR       = Path("outputs/t2i_flux_2") 
MODEL_NAME    = "flux"

# Define test prompts here.
# You may adjust the filename components (country/category/subcategory/variant) if needed.
PROMPTS = [
    {
    ...
    try:
        ...
        for node_out in outputs.values():
            for im in node_out.get("images", []):
                ...
                p = Path("output") / subfolder / filename  # ComfyUI default output dir
...
def build_from_template(prompt_text: str, out_prefix: Path, debug_once: List[bool]) -> Dict:
    """
    Keep all template parameters as-is.
    - Replace only the positive CLIPTextEncode(text != "") with prompt_text
    ...
    for node in wf.values():
        ...
        # Replace only the positive text (negative prompt in the template is kept as-is, usually an empty string)
    ...
    # One-time debug: verify that template parameters are being passed through as-is
    if not debug_once[0]:
        try:
            ...
            pos = [n for n in wf.values()
                   ...
                   and n.get("inputs",{}).get("text","").strip()!=""][0]
...
def main():
    ...
    for idx, item in enumerate(PROMPTS):
        ...
        try:
            ...
            hist = wait_history(pid, 120.0, 0.5)
